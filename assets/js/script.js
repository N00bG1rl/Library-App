...
document.getElementById('add').addEventListener('click', function() {
  ...
  if (title && author && pages) {
    ...
    /* Store content into data array for localhost */
    data.unread.push(title, author, pages);
    dataObjectUpdated();
  }
});

function renderBookList() {
  if (!data.unread.length && !data.readed.length) return;
...
function addItemtoDOM(title, author, pages, storage) {
  /* Get unread ul list from html */
  /* let list = document.getElementById('unreadList'); */

  let list = (storage) ? document.getElementById('readedList'):document.getElementById('unreadList');
